...
// The component uses the Link component from react-router-dom for navigation
 
export const Footer = () => {
  return (
    <footer className="py-16 bg-gray-950 px-12 flex justify-between gap-10 text-slate-200 text-sm mt-1 flex-wrap md:flex-nowrap">
      <Link
        to="/"
        ...
        <div className="flex">
          {socialLinks.map((link) => (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-slate-300 border border-gray-900 w-full h-full py-3.5 flex items-center justify-center transition-all hover:bg-white hover:text-gray-950"
            >
              {link.icon}
            </a>
          ))}
