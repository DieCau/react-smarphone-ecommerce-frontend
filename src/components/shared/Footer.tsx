...
export const Footer = () => {
  return (
    <footer className="py-16 bg-gray-950 px-12 flex justify-between-gap-10 text-slate-200 text-sm flex-wrap mt-10 md:flex-nowrap">
      <Link
        to="/"
        className={`text-2xl font-bold transition-all tracking-tighter text-white flex-1`}
      >
        Smart Phones
      </Link>
      ...
      <div className="flex flex-col gap-4 flex-1">
        ...
        <p className="text-xs leading-6">
          Todas las novedades que tenemos para Ti!
        </p>
        <div className="flex">
          {socialLinks.map((link) => (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-slate-300 w-full h-full py-5 flex items-center justify-center transition-all hover:bg-white hover:text-gray-950"
            >
              {link.icon}
            </a>
          ))}
